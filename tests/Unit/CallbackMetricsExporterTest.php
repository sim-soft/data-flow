...
class CallbackMetricsExporterTest extends TestCase
{
    ...
    public function test_no_op_when_all_closures_are_null(): void
    {
        ...
        $exporter->recordRowFailed('stage1', 'error');
    ...
    public function test_invokes_on_row_failed_closure_with_parameters(): void
    {
        $captured = [];
        $exporter = new CallbackMetricsExporter(
            onRowFailed: function (string $stageName, string $errorMessage) use (&$captured) {
                $captured[] = [$stageName, $errorMessage];
            },
        );

        $exporter->recordRowFailed('loader', 'Connection timeout');

        $this->assertSame([['loader', 'Connection timeout']], $captured);
    }
    ...
    public function test_only_configured_closures_are_invoked(): void
    {
        ...
        $exporter->recordRowFailed('stage', 'error');
